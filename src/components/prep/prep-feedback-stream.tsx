  ...
  return (
    <div className="rounded-md border bg-background p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex items-center gap-3">
            <CheckCircle2
              className={cn("h-5 w-5", scoreTone(feedback.score))}
            />
            <h3 className="text-lg font-semibold">{feedback.verdict}</h3>
            <span
              className={cn("text-lg font-bold", scoreTone(feedback.score))}
            >
              {feedback.score}/10
            </span>
          </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <FeedbackList title={ui("What worked")} items={feedback.strengths} />
        <FeedbackList
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-md bg-muted/40 p-4">
          <p className="text-sm font-medium">{ui("Structure")}</p>
          <p className="mt-2 text-sm text-muted-foreground">
        ...
        <div className="rounded-md bg-muted/40 p-4">
          <p className="text-sm font-medium">{ui("Sample answer")}</p>
          <p className="mt-2 whitespace-pre-wrap text-sm text-muted-foreground">
            {feedback.sampleAnswer || "Sample answer not generated yet."}
          </p>
          {feedback.needsUserVerification.length > 0 ? (
            <div className="mt-4">
              <p className="text-sm font-medium">{ui("Verify before using")}</p>
  ...
  return (
    <div className="rounded-md bg-muted/40 p-4">
      ...
      {items.length > 0 ? (
        ...
        <p className="mt-2 text-sm text-muted-foreground">{ui("No items.")}</p>
      )}
    </div>
  );
}
