      ...
      <div className="mx-auto w-full max-w-5xl flex-1 px-4 py-8 md:px-6">
        <Card className="mb-6">
          <CardContent className="flex flex-col items-center gap-3 py-8 text-center sm:flex-row sm:gap-4 sm:text-left">
            ...
            <div className="min-w-0">
              <h2 className="text-2xl font-bold">{ui("Practice complete")}</h2>
              <p className="mt-1 text-sm text-muted-foreground">
                {interviewTitle}
                {ui(
                  "— your session is saved. Review the report below, then plan your next run.",
                )}
              </p>
            </div>
        ...
        {sessionId ? (
          ...
          <p className="text-center text-sm text-muted-foreground">
            {ui("Find this run anytime under")}{" "}
            <Link
              href="/practices"
