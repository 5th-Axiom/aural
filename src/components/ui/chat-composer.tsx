    ...
    >
      ...
      <span
        className={cn(
          ...
          warn ? "text-amber-600 dark:text-amber-400" : "text-foreground/90",
        )}
      >
        {value}
      </span>
...
export function ChatComposer({
  ...
  const resizeTextarea = useCallback(() => {
    ...
    const next = Math.min(
      Math.max(el.scrollHeight, MIN_TEXTAREA_PX),
      MAX_TEXTAREA_PX,
    );
  ...
  return (
    ...
    >
      {voice?.pendingAudioUrl ? (
        <div className="mb-2 flex items-center gap-2 rounded-lg border bg-background/80 px-2 py-1.5">
          ...
          <Button
            ...
            aria-label={ui("Remove recording")}
          >
      ...
      <Textarea
        ...
        placeholder={
          aiTokensBlocked
            ? ui("Add AI tokens to submit answers and use coach voice…")
            : placeholder
        }
        disabled={
          disabled || Boolean(aiTokensBlocked) || (isGenerating && !onStop)
        }
        rows={1}
        className={cn(
        ...
        {isRecording ? (
          <>
            ...
            <Button
              ...
              aria-label={
                isGenerating ? ui("Stop generation") : ui("Send message")
              }
            >
              {isGenerating ? (
                ...
                <Button
                  ...
                  title={
                    sessionActions.coachDisabled
                      ? ui("Coach voice requires AI tokens")
                      : undefined
                  }
                >
